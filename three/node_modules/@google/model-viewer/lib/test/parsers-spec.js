...
const valueNode = (value, unit = null) => ({ type: 'value', value, unit });
suite('parsers', () => {
    suite('parseValues', () => {
        ...
        test('parses multiple CSS color strings', () => {
            expect(parseValues('red #00f #00ff00'))
                .to.be.eql([valueNode('red'), valueNode('#00f'), valueNode('#00ff00')]);
        });
        ...
        test('is resilient to awkward whitespace', () => {
            ...
            expect(parseValues(' 1px     \n2px #f00'))
                .to.be.eql([valueNode('1', 'px'), valueNode('2', 'px'), valueNode('#f00')]);
        });
    });
});
