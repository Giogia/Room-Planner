    ...
    suite('with a loaded model', () => {
        ...
        suite('align-model', () => {
            test('centers the model in the frame by default', () => {
                const offset = element[$scene].model.position;
            ...
            suite('origin alignment', () => {
                ...
                test('aligns model origin with the scene origin', () => {
                    ...
                    // -0 values as 0
                    const offset = element[$scene].model.position.clone();
                    expect(offset).to.be.deep.equal(ORIGIN_OFFSET);
                });
                test('places shadow under a non-centered model', () => {
                    const shadowSize = element[$scene].shadow.scale;
            ...
            suite('mixed values', () => {
                suite('two values', () => {
                    test('aligns x and y axes accordingly', async () => {
                        ...
                        await timePasses();
                        const offset = element[$scene].model.position.clone();
                        expect(offset).to.be.deep.equal(new Vector3(CENTER_OFFSET.x, ORIGIN_OFFSET.y, CENTER_OFFSET.z));
                    });
                });
                suite('three values', () => {
                    test('aligns x, y and z axes accordingly', async () => {
                        ...
                        await timePasses();
                        const offset = element[$scene].model.position.clone();
                        expect(offset).to.be.deep.equal(new Vector3(ORIGIN_OFFSET.x, CENTER_OFFSET.y, ORIGIN_OFFSET.z));
                    });
                });
            });
        });
