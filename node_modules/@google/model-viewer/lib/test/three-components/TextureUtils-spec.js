...
suite('TextureUtils', () => {
    ...
    suite('generating an environment map and skybox', () => {
        ...
        test('returns an environmentMap and skybox texture from url', async () => {
            ...
            expect(textureMatchesMeta(textures.skybox.texture, { mapping: 'Cube', url: EQUI_URL }))
                .to.be.ok;
            expect(textureMatchesMeta(textures.environmentMap, { mapping: 'Cube', url: EQUI_URL }))
                .to.be.ok;
        });
        ...
        test('returns an environmentMap and skybox texture from url with PMREM', async () => {
            textures = await textureUtils.generateEnvironmentMapAndSkybox(EQUI_URL, null, {
                pmrem: true,
            });
            expect(textures.skybox.texture.isTexture).to.be.ok;
            expect(textures.environmentMap.isTexture).to.be.ok;
        ...
        test('returns an environmentMap and skybox texture from an HDR url', async () => {
            textures = await textureUtils.generateEnvironmentMapAndSkybox(HDR_EQUI_URL, null, {
                pmrem: true,
            });
            expect(textures.skybox.texture.isTexture).to.be.ok;
            expect(textures.environmentMap.isTexture).to.be.ok;
    ...
    suite('dynamically generating environment maps', () => {
        test('creates a cubemap render target', async () => {
            const { environmentMap: texture } = await textureUtils.generateEnvironmentMapAndSkybox();
            expect(textureMatchesMeta(texture, { mapping: 'Cube', url: null }))
                .to.be.ok;
        });
        test('creates a cubemap render target with PMREM', async () => {
            const { environmentMap: texture } = await textureUtils.generateEnvironmentMapAndSkybox(null, null, {
                pmrem: true,
            });
